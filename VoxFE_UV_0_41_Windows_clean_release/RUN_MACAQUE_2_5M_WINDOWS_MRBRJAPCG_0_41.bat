...
set "CASE_DIR=%~dp0models\Macaque_2.5million"
...
set "RUN_SCRIPT=%CASE_DIR%\Script_autorun_windows_MRBRJAPCG_2_5M_0_41.txt"
set "SUMMARY=%CASE_DIR%\voxfe_solver_summary_windows_MRBRJAPCG_2_5M_0_41.json"
set "LOG=%CASE_DIR%\solver_windows_MRBRJAPCG_2_5M_0_41_stdout.log"
set "VALIDATION_LOG=%CASE_DIR%\voxfe_run_validation_log.json"
set "ELAPSED=%CASE_DIR%\windows_MRBRJAPCG_2_5M_0_41_elapsed_time.txt"
...
for /f %%g in ('powershell -NoProfile -Command "[math]::Round((Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory/1GB,1)"') do set "DETECTED_RAM_GB=%%g"
if "%DETECTED_RAM_GB%"=="" set "DETECTED_RAM_GB=unknown"
...
echo Solving Macaque 2.5 million on Windows with VoxFE 0.41 MRBRJAPCG
echo Case:      %CASE_DIR%
echo Python:    %PY%
echo Solver:    %SOLVER%
echo Method:    %METHOD%
...
echo Detected RAM: %DETECTED_RAM_GB% GB
echo Log:       %LOG%
echo Validation log:
echo   %VALIDATION_LOG%
echo.
powershell -NoProfile -Command "try { $ram=[double]'%DETECTED_RAM_GB%'; if ($ram -lt 64) { Write-Host 'WARNING: detected RAM is below 64 GB. This large model may be slow or memory-limited on native Windows.' } } catch { Write-Host 'WARNING: could not detect RAM. Monitor memory during this large run.' }"
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { & '%PY%' -u '%SOLVER%' 'Script_autorun_windows_MRBRJAPCG_2_5M_0_41.txt' --backend '%BACKEND%' --algorithm '%METHOD%' --threads auto --summary '%SUMMARY%' --progress-interval 50 --min-iter '%MIN_ITER%' 2>&1 | Tee-Object -FilePath '%LOG%'; exit $LASTEXITCODE }"
...
if "%SOLVE_EXIT%"=="0" if exist "%EXPORTER%" if exist "%CASE_DIR%\displacement.txt" (
    ...
    "%PY%" -u "%EXPORTER%" --case-dir "%CASE_DIR%" --script "%RUN_SCRIPT%" --summary "%CASE_DIR%\coordinate_export_summary_windows_MRBRJAPCG_2_5M_0_41.json" >> "%LOG%" 2>&1
)
