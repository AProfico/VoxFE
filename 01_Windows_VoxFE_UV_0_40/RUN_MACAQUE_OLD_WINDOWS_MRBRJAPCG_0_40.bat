...
set "RUN_SCRIPT=%CASE_DIR%\Script_autorun_windows_MRBRJAPCG_0_40.txt"
set "SUMMARY=%CASE_DIR%\voxfe_solver_summary_windows_MRBRJAPCG_0_40.json"
set "LOG=%CASE_DIR%\solver_windows_MRBRJAPCG_0_40_stdout.log"
set "ELAPSED=%CASE_DIR%\windows_MRBRJAPCG_0_40_elapsed_time.txt"
...
echo Solving Macaque old model on Windows with VoxFE 0.40 MRBRJAPCG
echo Case:      %CASE_DIR%
echo Python:    %PY%
echo Solver:    %SOLVER%
echo Method:    %METHOD%
...
cd /d "%CASE_DIR%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { & '%PY%' -u '%SOLVER%' 'Script_autorun_windows_MRBRJAPCG_0_40.txt' --backend '%BACKEND%' --algorithm '%METHOD%' --threads auto --summary '%SUMMARY%' --progress-interval 50 --min-iter '%MIN_ITER%' 2>&1 | Tee-Object -FilePath '%LOG%'; exit $LASTEXITCODE }"
set "SOLVE_EXIT=%ERRORLEVEL%"

if "%SOLVE_EXIT%"=="0" if exist "%EXPORTER%" (
    echo.
    echo Exporting loaded/unloaded coordinates...
    "%PY%" -u "%EXPORTER%" --case-dir "%CASE_DIR%" --script "%RUN_SCRIPT%" --summary "%CASE_DIR%\coordinate_export_summary_windows_MRBRJAPCG_0_40.json" >> "%LOG%" 2>&1
)
